# --- Project Days v3.1 integration header ---
# Auto-generated integration layer (minimal, non-destructive)
import os, time, json, requests

GAME_DIR = os.path.expanduser("~") + "/Project_Days"
ADMIN_KEY_FILE = os.path.join(GAME_DIR, "admin_key.txt")
FIREBASE_URL = "https://project-days-online-default-rtdb.asia-southeast1.firebasedatabase.app"

def to_lower_keys(d):
    """Recursively convert dict keys to lowercase to normalize saves."""
    if isinstance(d, dict):
        new = {}
        for k,v in d.items():
            new[k.lower() if isinstance(k,str) else k] = to_lower_keys(v)
        return new
    elif isinstance(d, list):
        return [to_lower_keys(i) for i in d]
    else:
        return d

def load_save_safe(path):
    try:
        with open(path,"r",encoding="utf-8") as f:
            data = json.load(f)
        data = to_lower_keys(data)
        # ensure uuid exists
        if 'uuid' not in data:
            import uuid
...
# Firebase minimal helpers (best-effort; silent failures)
def firebase_get(path):
    try:
        url = FIREBASE_URL.rstrip('/') + '/' + path.lstrip('/') + '.json'
...
# Admin check (local admin_key.txt)
def is_local_admin_player(player):
    try:
        if os.path.exists(ADMIN_KEY_FILE):
            with open(ADMIN_KEY_FILE,"r",encoding="utf-8") as f:
                key = f.read().strip()
            if key == "VYOKER-GM-11022025":
                return True
    except:
        pass
    return False

# Small helper to inject UUID lines into menu printing functions.
# We'll attempt to replace common patterns "Mode" lines and add "UUID" after them.
...
def create_new_game():
    ...
    time.sleep(0.6)
    slow("Tahun 2089... Indonesia dilanda kekacauan besar setelah wabah misterius menyebar dari proyek penelitian rahasia.", 0.02)
    slow("Berita dari pemerintah mengatakan seluruh kota lumpuh total.\n", 0.02)
    time.sleep(0.6)
...
try:
    print('UUID  : ' + (player.get('uuid') if isinstance(player, dict) else ''))
except Exception:
    try:
        print('UUID  : ' + (p.get('uuid') if isinstance(p, dict) else ''))
    except:
        pass
